...
This repo now has two supported application surfaces:
...
- Admin drafts and publishing are Supabase-backed.
- The player reads the latest published story from Supabase.
- `packages/story-content` remains the bundled fallback content path for builds and recovery cases.
...
If the bundled fallback story content needs to be updated from an approved export, promote that
exported `{ content, flowMap }` wrapper into `packages/story-content` before building:
...
Use `--dry-run` first when checking an export without changing `packages/story-content`. The command
validates the export, normalizes the content model, stores the exported flow map as
`defaultFlowMap`, and rewrites `packages/story-content/src/storyData.js` for player fallback builds.
